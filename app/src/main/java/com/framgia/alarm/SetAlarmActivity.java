...
public class SetAlarmActivity extends AppCompatActivity {
    ...
    private void mDeleteAlarm() {
        ...
        alertDialog.setButton(DialogInterface.BUTTON_POSITIVE, getString(R.string
            ...
            public void onClick(DialogInterface dialog, int which) {
                DatabaseHelper db = new DatabaseHelper(SetAlarmActivity.this);
                db.deleteAlarm(mId);
                db.closeDB();
                finish();
            }
    ...
    private void mPopulateViews(Alarm alarm) {
        mTimePicker.setCurrentHour(Integer.parseInt(new SimpleDateFormat("HH").format(new Date(alarm
                .getmTime()))));
        mTimePicker.setCurrentMinute(Integer.parseInt(new SimpleDateFormat("HH:mm").format(new
                Date(alarm.getmTime())).substring(3, 5)));
        if (alarm.getmStatus() == Constants.ON) mToggleOnOff.setChecked(true);
        else mToggleOnOff.setChecked(false);
        mLabel.setText(alarm.getmLabel().toString());
        for (int i = 0; i < 7; i++) {
        ...
        mButtonTonePicker.setText(RingtoneManager.getRingtone(this, Uri.parse(mToneUri)).getTitle(this));
    }
        ...
        try {
            if (requestCode == RESULT_PICK_TONE && resultCode == RESULT_OK
                ...
                if (uri != null) {
                    ...
                    mButtonTonePicker.setText(RingtoneManager.getRingtone(this, uri).getTitle(this));
                }
            } else {
    ...
    private void mSetAlarm() {
        ...
        if (mNewAlarm) {
            ...
            db.updateAlarm(new Alarm(calendar.getTimeInMillis(), status, label, uri, daySchedule)
                    , mId);
            db.closeDB();
        }
        finish();
