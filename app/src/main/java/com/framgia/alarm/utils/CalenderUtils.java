...
public class CalenderUtils {
    ...
    public static long updateEventInCalender(ContentResolver cr, Alarm alarm, int dayOfWek, int
            eventId) {
        Calendar c = Calendar.getInstance();
        c.setTimeInMillis(alarm.getTime());
        c.set(Calendar.DAY_OF_WEEK, dayOfWek);
        ...
        event.put(EVENT_STATUS, Constants.INT_ONE);
        event.put(HAS_ALARM, Constants.INT_ONE);
        Uri uri = ContentUris.withAppendedId(CalendarContract.CONTENT_URI, eventId);
        return cr.update(uri, event, null, null);
    }
